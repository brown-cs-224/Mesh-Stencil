...
3. How to run your code to reproduce the results shown in your submitted images. _This must not require any modifications to your source code: instead, use command-line arguments (see: [Final Grading](#final-grading))._
...
- `<arg1>` to `<arg4>` are extra parameters for your methods. Details are documented in the stencil code's main function.
  - If you hard-code any of these (which might happen for `remesh` and `denoise`), document them as well.
